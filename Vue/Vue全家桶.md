...
<img src="Untitled.assets/@click.png"  width="90%">
...
**B.添加路由链接:<router-link>是路由中提供的标签，默认会被渲染为 a 标签，to 属性默认被渲染为 href 属性，**
...
//path 表示 路由匹配的 hash 地址，component 表示路由规则对应要展示的组件对象
...
小结：
Vue Router 的使用步骤还是比较清晰的，按照步骤一步一步就能完成路由操作
A.导入 js 文件
B.添加路由链接
...
补充：
路由`重定向`：可以通过路由重定向为页面设置默认展示的组件
在路由规则中添加一条路由规则即可，如下：
var myRouter = new VueRouter({
//routes 是路由规则数组
routes: [
//path 设置为/表示页面最初始的地址 / ,redirect 表示要被重定向的新地址，设置为一个路由即可
{ path:"/",redirect:"/user"},
{ path: "/user", component: User },
{ path: "/login", component: Login }
]
...
补充：
如果使用$route.params.id 来获取路径传参的数据不够灵活。 1.我们可以通过 props 来接收参数
var User = {
props:["id"],
...
2.还有一种情况，我们可以将 props 设置为对象，那么就直接将对象的数据传递给
组件进行使用
var User = {
props:["username","pwd"],
...
案例思路：
1).先将素材文件夹中的 11.基于 vue-router 的案例.html 复制到我们自己的文件夹中。
...
5).当我们访问页面的时候，默认需要展示刚刚创建的 app 根组件，我们可以
创建一个路由对象来完成这个事情,然后将路由挂载到 Vue 实例对象中即可
...
补充：到此为止，基本的 js 代码都处理完毕了，我们还需要设置一个路由占位符
...
点击创建项目，如下图所示
第一步，设置项目名称和包管理器
![](Untitled.assets/创建vuex项目01.png)
第二步，设置手动配置项目
![](Untitled.assets/创建vuex项目02.png)
第三步，设置功能项
![](Untitled.assets/创建vuex项目03.png)
![](E:/itheima/02-配套课件/阶段五： Vue.js 项目实战开发/阶段五：Vue.js 项目实战资料/阶段五：Vue.js 项目实战资料/Vue-2019 版/vue 电商/10.vuex（5 章 1-2 小节）/笔记/images/创建 vuex 项目 04.png)
第四步，创建项目
![](Untitled.assets/创建vuex项目05.png)
...
# @vue/cli 和 vue 和 element

- ![image-20220509120708049](Vue全家桶.assets/image-20220509120708049.png)
